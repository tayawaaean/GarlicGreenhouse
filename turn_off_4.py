...
# Endpoint for turning off relay number 4
endpoint = "http://10.40.0.176:80/turn_off/4"
        ...
        if schedule_document:
            ...
            try:
                ...
                if response.status_code == 200:
                    # Update the relay state in MongoDB to turn off the light
                    light_state_collection.update_one(
                        {'relay_num': 4},  # Relay number 4
                        {'$set': {'state': False}},
                        upsert=True  # Create a new document if it doesn't exist
                    )
                    print("Relay state updated in MongoDB for relay 4")
